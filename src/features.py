...
def cqcc(
    ...
    ВАЖНО #2 (найдено при проверке диапазона частот, которые CQCC
    реально покрывает): у spafe есть собственный внутренний параметр
    nfft (здесь - internal_nfft, чтобы не путать с нашим n_fft),
    отвечающий за то, до каких низких частот "дотягивается" анализ.
    Чем ниже частота при фиксированном Q, тем длиннее нужно окно, а
    internal_nfft - потолок на эту длину; частоты, которым нужно окно
    длиннее, молча выбрасываются. С дефолтом spafe (internal_nfft=512)
    при bins_per_octave=96 реально выживают только частоты
    ~11930-22038 Гц - меньше половины спектра, преимущественно верхняя
    его часть. internal_nfft=8192 (новый дефолт здесь) расширяет это
    до ~746-22038 Гц (макс. окно ~185 мс) - разумный компромисс между
    охватом и длиной окна. Чтобы дотянуться до низкого f0=120 Гц по
    умолчанию, потребовалось бы окно больше секунды - непрактично для
    данного пайплайна.
